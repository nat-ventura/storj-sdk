...
Make sure the new directory matches the directory that would appear in a project's `node_modules` folder. For instance, if you were adding `https://github.com/Storj/mongodb-adapter`, the submodule would actually need to be placed in `vendor/storj-mongodb-adapter`.
